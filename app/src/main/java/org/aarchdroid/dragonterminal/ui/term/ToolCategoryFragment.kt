...
class ToolCategoryFragment : Fragment() {
    private var categoryName: String = ""
    private var bannerResId: Int = 0
    private var statsToolsCount: String = "0"
    private var categoryDbKey: String = ""

    private var adapter: ToolAdapter? = null
    private var processingTools = HashSet<String>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        arguments?.let {
            ...
            bannerResId = it.getInt(ARG_BANNER, 0)
            statsToolsCount = it.getString(ARG_STATS, "0")
            categoryDbKey = it.getString(ARG_DB_KEY, categoryName.lowercase().replace(" ", "_"))
        }
    }

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        return inflater.inflate(R.layout.dco_list_scaffold, container, false)
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)

        view.findViewById<TextView>(R.id.title).text = categoryName
        view.findViewById<ImageView>(R.id.banner).setImageResource(bannerResId)
        view.findViewById<TextView>(R.id.stats_tools).text = statsToolsCount

        val list = view.findViewById<RecyclerView>(R.id.tool_list)
        ...
        adapter = ToolAdapter(tools, object : ToolAdapter.OnToolClickListener {
            ...
            override fun onUninstallClick(toolKey: String) = this@ToolCategoryFragment.onUninstallClick(toolKey)
            override fun onLaunchTool(toolKey: String) = this@ToolCategoryFragment.onLaunchTool(toolKey)
        })
    ...
    private fun updateStatsSize() {
        ...
        val sizeView = view.findViewById<TextView>(R.id.stats_size)
        val toolsView = view.findViewById<TextView>(R.id.stats_tools)
        val compactView = view.findViewById<TextView>(R.id.stats_compact)
        val downView = view.findViewById<TextView>(R.id.stats_downloaded)
        val updView = view.findViewById<TextView>(R.id.stats_updated)
    ...
    private fun processExitFiles() {
        try {
            val stateDir = File(requireContext().filesDir, "install-state")
    ...
    fun processInstallTool(toolKey: String) {
        ...
        if (installCmd != null) {
            ...
            File(requireContext().filesDir, "install-state").mkdirs()
            try {
                File(requireContext().filesDir, "install-state/${nk}.pending").createNewFile()
            } catch (_: Exception) {}
            ...
            Toast.makeText(requireContext(), "No install command for $toolKey", Toast.LENGTH_SHORT).show()
            processingTools.remove(nk)
        }
    }
    ...
    fun onUninstallClick(toolKey: String) {
        ...
        if (cmd != null) {
            ...
            Toast.makeText(requireContext(), "No uninstall command for $toolKey", Toast.LENGTH_SHORT).show()
            processingTools.remove(nk)
        }
    }
    ...
    private fun runHackCmd(cmd: String, iconResId: Int = 0, toolKey: String? = null) {
        ...
        if (hasRoot != true) {
            Toast.makeText(requireContext(), "Root no detectado", Toast.LENGTH_SHORT).show()
            return
        }
    ...
    private fun setupDynamicSizing(list: RecyclerView) {
        if (layoutSet) return
        layoutSet = true
        ...
        val decorView = view ?: return
        decorView.viewTreeObserver.addOnPreDrawListener(
            object : ViewTreeObserver.OnPreDrawListener {
                private var done = false
                override fun onPreDraw(): Boolean {
                    if (done) return true
                    val rv = view?.findViewById<RecyclerView>(R.id.tool_list) ?: return true
    ...
    private fun setupScrollIndicator(rv: RecyclerView) {
        ...
        val indicator = root.findViewById<View>(R.id.scroll_indicator) ?: return
        val thumb = root.findViewById<View>(R.id.scroll_thumb) ?: return
    ...
    companion object {
        ...
        fun newInstance(categoryName: String, bannerResId: Int, statsToolsCount: String, dbKey: String? = null): ToolCategoryFragment {
            return ToolCategoryFragment().apply {
                arguments = Bundle().apply {
                    putString(ARG_CATEGORY, categoryName)
                    putInt(ARG_BANNER, bannerResId)
                    putString(ARG_STATS, statsToolsCount)
                    putString(ARG_DB_KEY, dbKey ?: categoryName.lowercase().replace(" ", "_"))
                }
            }
        }
    }
