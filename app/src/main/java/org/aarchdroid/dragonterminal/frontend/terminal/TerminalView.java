...
import org.aarchdroid.dragonterminal.backend.TerminalBuffer;
import org.aarchdroid.dragonterminal.backend.TerminalEmulator;
...
public final class TerminalView extends View {
    ...
    public void scrollPage(boolean up) {
        if (mEmulator == null) return;
        if (mEmulator.isMouseTrackingActive()) return;
        if (mEmulator.isAlternateBufferActive()) {
            handleKeyCode(up ? KeyEvent.KEYCODE_DPAD_UP : KeyEvent.KEYCODE_DPAD_DOWN, 0);
            return;
        }
        int pageRows = Math.max(1, mEmulator.mRows - 1);
        mTopRow = Math.min(0, Math.max(
                -(mEmulator.getScreen().getActiveTranscriptRows()),
                mTopRow + (up ? -pageRows : pageRows)));
        if (!awakenScrollBars()) invalidate();
    }
    ...
    public void updateSize() {
        ...
        if (mEmulator == null || (newColumns != mEmulator.mColumns || newRows != mEmulator.mRows)) {
            int cellWidth = (int) mRenderer.mFontWidth;
