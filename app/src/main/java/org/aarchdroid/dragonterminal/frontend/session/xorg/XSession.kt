...
import org.aarchdroid.dragonterminal.*
import org.aarchdroid.dragonterminal.frontend.session.xorg.client.XSessionData
...
class XSession constructor(private val mActivity: Activity, val mSessionData: XSessionData) : NeoXorgViewClient {
        ...
        if (!isKeyboardWithoutTextInputShown()) {
            ...
            runOnUiThread(Runnable {
                if (keyboard == 0) {
                    ...
                    inputManager.showSoftInput(mSessionData.glView, InputMethodManager.SHOW_FORCED)
                    mActivity.window.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_STATE_VISIBLE)
                } else {
            ...
            runOnUiThread {
                ...
                inputManager.hideSoftInputFromWindow(mSessionData.glView!!.windowToken, 0)
            }
        }
        mSessionData.glView!!.callNativeScreenKeyboardShown(if (isKeyboardWithoutTextInputShown()) 1 else 0)
    }
    ...
    override fun hideScreenKeyboard() {
        ...
        mSessionData.videoLayout!!.removeView(mSessionData.screenKeyboard)
        mSessionData.screenKeyboard = null
        mSessionData.glView!!.isFocusableInTouchMode = true
        mSessionData.glView!!.isFocusable = true
        mSessionData.glView!!.requestFocus()
    }

    override fun updateScreenOrientation() {
        var rotation: Int = mActivity.windowManager.defaultDisplay.rotation
    ...
    override fun setSystemMousePointerVisible(visible: Int) {
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.N) {
            mSessionData.glView?.pointerIcon = android.view.PointerIcon.getSystemIcon(mActivity,
