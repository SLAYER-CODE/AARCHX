...
import org.aarchdroid.dragonterminal.frontend.terminal.extrakey.combine.CombinedSequence
...
abstract class IExtraButton : View.OnClickListener {
    ...
    companion object {
        ...
        var SELECTED_TEXT_COLOR = 0xFFFF0000.toInt()
        ...
        fun sendKey(view: View, keyName: String) {
            ...
            val terminalView = if (view.parent is View) {
                (view.parent as View).findViewById<TerminalView>(R.id.terminal_view)
            } else {
                view.findViewById<TerminalView>(R.id.terminal_view)
            }
            if (keyCode > 0) {
