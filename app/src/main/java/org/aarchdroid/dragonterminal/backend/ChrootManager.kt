...
[ ! -x "${D}CHROOT_BASE${D}SHELL" ] && SHELL="/bin/bash"
                export HOME=/root
                export AARCHDROID_TERMINAL=1
                export CANVAS_SOCKET=canvas-display
                export CAMERA_SOCKET=cam-0
                case "${D}SHELL" in
  */bash) exec chroot "${D}CHROOT_BASE" /bin/sh -c "cd /root && export AARCHDROID_TERMINAL=1 && export CANVAS_SOCKET=canvas-display && export CAMERA_SOCKET=cam-0 && exec ${D}SHELL --rcfile /root/.bashrc";;
  *) exec chroot "${D}CHROOT_BASE" /bin/sh -c "cd /root && export AARCHDROID_TERMINAL=1 && export CANVAS_SOCKET=canvas-display && export CAMERA_SOCKET=cam-0 && exec ${D}SHELL" 2>/dev/null;;
esac
"""
    ...
    fun getEntryCommand(): String {
        return "export HOME=/root; export AARCHDROID_TERMINAL=1; export CANVAS_SOCKET=canvas-display; export CAMERA_SOCKET=cam-0; SHELL=\$(grep \"^root:\" $CHROOT_BASE/etc/passwd | cut -d: -f7); [ -z \"\$SHELL\" ] && SHELL=/bin/bash; [ ! -x $CHROOT_BASE/\$SHELL ] && SHELL=/bin/bash; case \"\$SHELL\" in */bash) exec chroot $CHROOT_BASE /bin/sh -c \"cd /root && export AARCHDROID_TERMINAL=1 && export CANVAS_SOCKET=canvas-display && export CAMERA_SOCKET=cam-0 && exec \$SHELL --rcfile /root/.bashrc\";; *) exec chroot $CHROOT_BASE /bin/sh -c \"cd /root && export AARCHDROID_TERMINAL=1 && export CANVAS_SOCKET=canvas-display && export CAMERA_SOCKET=cam-0 && exec \$SHELL\";; esac"
    }
